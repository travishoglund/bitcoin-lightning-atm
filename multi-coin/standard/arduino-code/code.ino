...
float coinValues[4] = {0.01, 0.05, 0.1, 0.25};
int charge = 10; // % you will charge people for service, set in LNbits extension
float maxAmount = 5.00; // max amount per withdraw
...
void loop() {
  ...
  // Identified the coin by number of pulses
  if(impulseCount > 0 && (millis() - lastInterrupt > 1000)) {
    total = total + coinValues[impulseCount - 2]; // Pulses start at 2 for pennies, array starts at 0
    impulseCount = 0;
...
void showATMStartScreen() {
  ...
  printMessage("Bitcoin ATM", String(charge) + "% charge (" + currencyATM + ")", "TRADE FIAT FOR SATS!", TFT_WHITE, TFT_BLACK);
}
